...
#2021/11/20
flags.DEFINE_string('label_map_pbtxt', "", 'File path to label_map.pbtxt.')
flags.DEFINE_string('early_stopping', "map", 'map:mAPEarlyStopping, fvalue:FvalueEarlyStopping.')
flags.DEFINE_string('eval_dir', None, 'Location of eval_dir')
flags.DEFINE_integer('patience', 10,
                     'The number of epochs to EarlyStopping')
...
def main(_):
  ...
  if os.path.exists(FLAGS.label_map_pbtxt) ==False:
    raise Exception("Not found " + FLAGS.label_map_pbtxt)
  ...
  if patience > 0:
    if FLAGS.early_stopping == "map":
      early_stopping = mAPEarlyStopping(patience)
    elif FLAGS.early_stopping == "fvalue":
      early_stopping = FvalueEarlyStopping(patience)
    else:
      print("Invalid FLAGS.early_stopping {}".format(FLAGS.early_stopping))
  # </added>
  ...
  if FLAGS.strategy == 'tpu':
    ...
    def get_estimator(global_batch_size):
      ...
      params['eval_dir']   = FLAGS.eval_dir     #2021/11/14
